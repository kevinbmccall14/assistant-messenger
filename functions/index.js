...
exports.dialogflowFulfillment = functions.https.onRequest((request, response) => {
  ...
  function unknown (app) {
    app.ask('What the fuck did you say?');
  }
  ...
  function addContact(app) {
    ...
    return admin.database().ref('/contacts').child(contact).set({phone_number: number}).then(() => {
  ...
  function sendText(app) {
    ...
    admin.database().ref('/contacts').once('value', snapshot => {
      ...
      if(number) {
        ...
        response.status(404).send('Phone number not found in contacts');
        return;
      }
    })
